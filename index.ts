...
app.post("/createSignature", async (req: Request, res: Response) => {
  ...
  let verifierDid = await Custodian.getDID(request.verifierDid);
  delete verifierDid.kid;
...
app.get("/verifySignature", async (req: Request, res: Response) => {
  const request: verificationRequest = req.body as unknown as verificationRequest;
